...
class MMAController(Controller):
    ...
    def choose_model(self, x):
        ...
        for i, model in enumerate(self.models):
            ...
            self.x_n[i][0] = self.x_n[i][0] + ((self.x_ndot_prev[i][0] + x_ndot[0]) / 2 * self.Tp)
            #print(self.x_n[0])
            self.x_n[i][1] = self.x_n[i][1] + ((self.x_ndot_prev[i][1] + x_ndot[1]) / 2 * self.Tp)
            self.x_n[i][2] = x_ndot[0]
            self.x_n[i][3] = x_ndot[1]
            self.x_ndot_prev[i] = x_ndot.ravel()
            err[i] = (abs(x[0] - self.x_n[i][0]) + abs(x[1]-self.x_n[i][1])) / 2
        print('model nr')
        print(err)
